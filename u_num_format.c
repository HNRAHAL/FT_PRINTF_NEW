...
/*   Updated: 2025/10/29 16:09:28 by hrahal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	u_number_case(va_list *list, t_format *flg, int w_nbr, int p_nbr)
{
	...
	if (w_nbr > len && p_nbr > len)
	{
		...
		count += u_edge_case1(c, w_nbr, p_nbr);
	return (count);
}
...
int	u_number_case_one(t_format *flg, int c, int w_nbr, int p_nbr)
{
	...
	w_nbr -= len;
	if (flg->minus_flag)
		count += u_edge_case2(c, w_nbr, p_nbr);
	else if (flg->zero_flag && p_nbr != -1)
		count += u_edge_case3(c, w_nbr, p_nbr);
	else if (flg->zero_flag)
	{
		...
		count += u_edge_case4(c, w_nbr, p_nbr);
	return (count);
}
