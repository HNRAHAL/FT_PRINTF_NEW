...
/*   Updated: 2025/10/29 17:14:39 by hrahal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	x_upper_edge_case2(t_format *flg, int num, int w_nbr, int p_nbr)
{
	...
	if ((flg->minus_flag && flg->hash_flag) || (flg->zero_flag
		...
		count += x_upper_edge_case3(str, w_nbr, len);
	else if (flg->zero_flag && p_nbr != -1)
		count += x_upper_edge_case4(str, w_nbr, len);
	else if (flg->zero_flag)
		count += x_upper_edge_case5(str, w_nbr, len);
	else
