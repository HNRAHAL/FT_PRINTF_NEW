...
int number_case(va_list *list, t_format *flg, int w_nbr, int p_nbr)
{
    ...
    if(w_nbr > len && p_nbr > len)
    {
        ...
        count += number_case_one(flg, c, len, w_nbr);
...
int number_case_one(t_format *flg, int c, int len, int w_nbr)
{
    ...
    if((flg->plus_flag && flg->minus_flag) || (flg->space_flag && flg->minus_flag)||
        (flg->plus_flag && flg->zero_flag) || (flg->space_flag && flg->zero_flag))
        count += number_combined_flags(flg, w_nbr, c);
    else if((flg->plus_flag) || (flg->minus_flag)||(flg->space_flag )||(flg->zero_flag))
        count += number_individual_flags(flg, w_nbr, c);
    else
    {
        while (w_nbr-- > 0)
            count += b_ft_putchar(' ');
...
int number_case_three(t_format *flg, int c, int w_nbr, int p_nbr)
{
    ...
    if((flg->minus_flag && flg->plus_flag)||(flg->minus_flag && flg->space_flag))
