...
/*   Updated: 2025/10/29 16:47:29 by hrahal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	x_lower_number_case(va_list *list, t_format *flg, int w_nbr, int p_nbr)
{
	...
	if (w_nbr > len && p_nbr > len)
		count += x_lower_edge_case1(flg, num, w_nbr, p_nbr);
	else if (w_nbr > len && w_nbr != -1)
		count += x_lower_number_case_one(flg, p_nbr, num, w_nbr);
		...
		int w_nbr)
{
	...
	if (num == 0)
	{
		...
		count += x_lower_edge_case2(flg, num, w_nbr, p_nbr);
	free(str);
	return (count);
}
